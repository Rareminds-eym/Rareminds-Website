...
serve(async (req) => {
  ...
  const downloadType = record?.download_type || "Unknown PDF";

  const html = `
    <h3>New PDF Download - ${downloadType}</h3>
    ...
    ${submittedAt !== 'N/A' ? `<p><strong>Downloaded at:</strong> ${submittedAt}</p>` : ''}
  `;

  const payload = {
    from: "Rareminds <no-reply@rareminds.in>",
    to: ["marketing@rareminds.in"],
    subject: `New Download: ${downloadType}`,
    html,
  };
