...
const Gallery: React.FC = () => {
  ...
  const videos = [
    { id: 'PR0I8bd5cJg', title: 'Rareminds Video - Part 1' },
    { id: 'E5WHb_lKgwg', title: 'Rareminds Video - Part 2' }
  ];

  const getVideoUrl = (videoId: string) => 
    `https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&controls=1&modestbranding=1&rel=0&showinfo=0&enablejsapi=1`;
  
  const shareUrl = window.location.href;
  const videoTitle = "Rareminds Video Gallery";

  const handleCopyLink = async (videoIndex: number) => {
    try {
      await navigator.clipboard.writeText(shareUrl);
      setCopySuccess(prev => ({ ...prev, [videoIndex]: true }));
      setTimeout(() => setCopySuccess(prev => ({ ...prev, [videoIndex]: false })), 2000);
    } catch (err) {
      console.error('Failed to copy:', err);
    }
  };

  const handleShare = (platform: string, videoIndex: number) => {
    ...
    setShowShareMenu(prev => ({ ...prev, [videoIndex]: false }));
  };

  const toggleFullscreen = (videoIndex: number) => {
    setIsFullscreen(prev => ({ ...prev, [videoIndex]: !prev[videoIndex] }));
  };
              ...
              <div className="relative bg-white rounded-3xl shadow-2xl border-2 border-gray-100 overflow-hidden transition-all duration-500 hover:shadow-3xl hover:border-blue-200">
                ...
                <div className="absolute top-4 right-4 flex gap-2 z-10">
                  ...
                  <div className="relative">
                    <button
                      onClick={() => setShowShareMenu(prev => ({ ...prev, [index]: !prev[index] }))}
                    ...
                    {showShareMenu[index] && (
                      <div className="absolute top-full right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl border-2 border-gray-100 p-4 animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        <div className="space-y-2">
                          <button
                            onClick={() => handleShare('facebook', index)}
                          ...
                          <button
                            onClick={() => handleShare('twitter', index)}
                          ...
                          <button
                            onClick={() => handleShare('linkedin', index)}
                            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-blue-50 transition-colors duration-200"
                          >
                            <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                          ...
                          <button
                            onClick={() => handleShare('email', index)}
