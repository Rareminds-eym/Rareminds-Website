                ...
                >
                    ...
                    <div className={`mb-4 p-4 rounded-lg border-2 ${hasValidDate ? 'bg-red-50 border-red-100' : 'bg-amber-50 border-amber-200'}`}>
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                            📍 {currentEvent.location_metadata?.address || 'Location TBD'} | ⏱️ {currentEvent.duration != null ? `${Math.floor(currentEvent.duration / 60)}h${currentEvent.duration % 60 > 0 ? ` ${currentEvent.duration % 60}m` : ''}` : 'Duration TBD'}
                        </p>
                    </div>
