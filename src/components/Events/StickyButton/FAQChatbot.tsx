// src/components/Events/StickyButton/FAQChatbot.tsx
...
const FAQChatbot: React.FC<FAQChatbotProps> = ({ isVisible, onClose, eventFAQs, eventTitle }) => {
  ...
  const [chatLog, setChatLog] = useState<ChatEntry[]>([
    {
      type: "bot",
      message: eventTitle 
        ? `Hello! 👋 I'm your FAQ assistant for "${eventTitle}". How can I help you with this event today?`
        : "Hello! 👋 I'm your Events FAQ assistant. How can I help you with our events today?",
    },
  ]);
  ...
  const findAnswer = (question: string): string => {
    // First, check event-specific FAQs if available
    if (eventFAQs && eventFAQs.length > 0) {
    ...
    return eventTitle 
      ? `I'm sorry, I couldn't find an answer to that question in the FAQs for "${eventTitle}". Please feel free to contact the event organizer for more specific information about this event.`
      : "I'm sorry, I couldn't find an answer to that question. Please feel free to contact our events team for more specific information about our events and services.";
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (userInput.trim()) {
      // Filter suggestions based on user input from event-specific FAQs
      const filteredSuggestions: string[] = [];
      if (eventFAQs && eventFAQs.length > 0) {
        for (const faq of eventFAQs) {
          if (faq.question.toLowerCase().includes(userInput.toLowerCase())) {
            filteredSuggestions.push(faq.question);
          }
        }
      }
          ...
          >
            <div className="bg-red-600 p-4 flex items-center justify-between shrink-0">
              <h2 className="text-lg font-semibold text-white">
                {eventTitle ? `${eventTitle} - FAQ` : 'Events FAQ Assistant'}
              </h2>
              <button
                onClick={onClose}
            ...
            <div className="p-4 flex-1 flex flex-col min-h-0">
                  ...
                  <motion.div
                    ...
                    className={`p-3 rounded-lg max-w-[85%] shadow-sm ${
                      entry.type === "user"
                        ...
                        : "bg-gradient-to-b from-red-100 to-red-200 self-start mr-auto"
                    }`}
                  >
                    {entry.message}
                  </motion.div>
              ...
              <form onSubmit={handleSubmit} className="flex gap-2">
                <div className="flex-1 relative">
                  <input
                    type="text"
                    className="w-full border border-gray-200 p-2.5 pr-12 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400"
                    value={input}
                    onChange={handleInputChange}
                    placeholder={eventTitle ? `Ask about ${eventTitle}...` : "Ask about events..."}
                  />
                  <button
                    type="submit"
