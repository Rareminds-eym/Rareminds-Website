...
const YouTubeFeed = () => {  
  ...
  // Only using "teacher" as keyword to be very specific
  const teacherKeyword = 'teacher';
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        setLoading(true);        // Search specifically for "teacher" videos to improve initial results
        const res = await fetch(
          `https://www.googleapis.com/youtube/v3/search?key=${apiKey}&channelId=${channelId}&part=snippet,id&order=date&maxResults=${maxResults}&q=${teacherKeyword}`
        );
        ...
        const data = await res.json();        if (data.items) {
          // Strict filtering to ensure we ONLY get videos with "teacher" in the title
          const filteredVideos = data.items.filter(
            (item: VideoItem) => {
              ...
              // Only include videos that have "teacher" in the title
              return title.includes(teacherKeyword);
            }
          );
  ...
  return (
    <div className="youtube-container max-w-5xl mx-auto flex flex-col items-center p-8">
      ...
      {loading && <p>Loading teacher videos...</p>}
